...
const module = device.createShaderModule({
  code: `
    ...
    fn main(
      ...
      (*dst_ball).charge = src_ball.charge;

      let collision_entropy = 0.4;
      // Ball/Wall collision
      if((*dst_ball).position.x - (*dst_ball).radius < 0.) {
        (*dst_ball).position.x = (*dst_ball).radius;
        (*dst_ball).velocity.x = -(*dst_ball).velocity.x * collision_entropy;
      }
      if((*dst_ball).position.y - (*dst_ball).radius < 0.) {
        (*dst_ball).position.y = (*dst_ball).radius;
        (*dst_ball).velocity.y = -(*dst_ball).velocity.y * collision_entropy;
      }
...
for (let i = 0; i < NUM_BALLS; i++) {
  inputBalls[i * 6 + 0] = random(minRadius, maxRadius); // radius
  inputBalls[i * 6 + 1] = random(-1, 1) // charge
  inputBalls[i * 6 + 2] = random(0, ctx.canvas.width); // pos x
  inputBalls[i * 6 + 3] = random(0, ctx.canvas.height); // pos y
  inputBalls[i * 6 + 4] = random(-100, 100); // vel x
  inputBalls[i * 6 + 5] = random(-100, 100); // vel y
}
let outputBalls;
...
while (true) {
  ...
  if (render !== 0) {
    ...
    ctx.arc(0, 0, ctx.canvas.width, ctx.canvas.height);
  }
